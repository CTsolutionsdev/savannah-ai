...
export default function EssayOutliner({ context }: Props) {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // Create a custom submit with the topic as the message
    const fakeEvent = {
      preventDefault: () => {},
    } as React.FormEvent;
    // Use setMessages to add user message, then trigger
    setMessages([...messages, { id: Date.now().toString(), role: 'user', content: topic }]);
  };

  // Simpler approach: use chat directly
  const { messages: chatMessages, input, handleInputChange, handleSubmit: chatSubmit, isLoading: chatLoading } = useChat({
    api: '/api/chat',
    body: { mode: 'essay', context },
    id: 'essay',
  });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <form onSubmit={chatSubmit} className="border-t border-gray-200 dark:border-gray-800 px-4 py-3">
