    ...
    <div className="flex flex-col h-full">
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 text-center max-w-md">
              Paste case text or enter a case name. I&apos;ll generate an IRAC brief: Issue, Rule, Application, Conclusion.
            </p>
            <p className="text-xs text-[#73C2E1] mt-3">Built for SULC 🐆</p>
          </div>
        ) : (
          <div className="space-y-4 max-w-3xl mx-auto">
            {messages.map((m) => (
              <div key={m.id} className={`${m.role === 'user' ? 'text-right' : ''}`}>
                {m.role === 'user' ? (
                  <div className="inline-block bg-[#73C2E1] text-white rounded-2xl px-4 py-3 text-sm max-w-[80%] text-left">
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-200 dark:border-gray-800 px-4 py-3">
        <div className="flex gap-2 max-w-3xl mx-auto">
          <input value={input} onChange={handleInputChange} placeholder="Case name or paste case text (e.g. Marbury v. Madison)..." className="flex-1 px-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-[#73C2E1]" disabled={isLoading} />
